...
class TshirtecommerceUpdater
{
	public function printing()
	{
		...
		$printings = $this->getData('printings', 'data');
		...
		$settings = $this->getData('settings', 'data');
		...
		$this->saveData('printings', $printings, 'data');
		/* End move printing */

		/* update printing from products */
		$products 	= $this->getData('products', 'data');
		$changed 	= false;
		...
		if ($changed == true) {
			$this->saveData('products', $products, 'data');
		}
	}

	public function getData($file_name, $folder = '')
	{
		if (empty($folder)) {
			$file = _PS_ROOT_DIR_.'/tshirtecommerce/'.$file_name.'.json';
		} else {
			$file = _PS_ROOT_DIR_.'/tshirtecommerce/'.$folder.'/'.$file_name.'.json';
		}
	...
	public function saveData($file_name, $params = array(), $folder = '')
	{
		if (empty($folder)) {
			$file = _PS_ROOT_DIR_.'/tshirtecommerce/'.$file_name.'.json';
		} else {
