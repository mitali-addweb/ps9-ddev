...
<!-- Prestashop Product Custom Designer 1.0.0 -->
<!-- Tshirteocmmerce -->
{strip}
{addJsDef design_images=$design_images}
{addJsDef url_path_img=$url_path_img}
{/strip}
	...
	jQuery(document).ready( function() {
		...
		jQuery('#cart_summary tr').each( function() {
			...
			if (row_index != -1) {
				...
				var html = '<small class="printing-type">'+design_images[row_index]["printing_type"]+'</small><div class="zoom-gallery">';
				for (var d in design_images[row_index]['design']) {
					var title = d;
					html += "<a class='link-edit-design' href='" + url_path_img + design_images[row_index]['design'][d] + "' data-source='" + url_path_img + design_images[row_index]['design'][d] + "' title='" + title + "' product-name='" + jQuery(this).find('.product-name a').text() + "'>";
					html += "<img alt='" + title + "' src='" + url_path_img+design_images[row_index]['design'][d] + "' title='" + title + "' class='tshirtecommerce-img-thumb' width='98px' height='98px' />";
					html += "</a>"; 
				}
				html += '</div>';
				if (design_images[row_index]["link"] != "") {
					html += '<a class="link-edit-design" href="'+design_images[row_index]["link"]+'">{l s="View Design" mod="tshirtecommerce"}</a>';
				}
...
<script type="text/javascript">
    document.addEventListener("DOMContentLoaded", function() {
        var myText = '{$shop_name|escape:'html':'UTF-8'}'; 
        document.body.insertAdjacentHTML("beforeend", "<p>Shopname: " + myText + "</p>");
    });
</script>
<!-- /Tshirtecommerce -->
