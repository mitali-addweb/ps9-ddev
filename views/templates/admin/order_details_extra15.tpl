...
<script type="text/javascript">
	jQuery(document).ready( function() {
		jQuery('#orderProducts tr').each( function() {
			var row_idx = jQuery(this).index('#orderProducts tbody tr'), html = '', link = '';
			if (row_idx != -1 && jQuery(this).attr('id') != 'new_product' && jQuery(this).attr('id') != 'new_invoice') {
				{foreach from=$eproducts key=row item=product}
					console.log(row_idx);
					if (row_idx-1 == {$row}) {
						{if isset($product.print_type)}
							html += '<span class="printing-type">{$product.print_type}</span>';
						{/if}
						html += '<div class="zoom-gallery">';
						{assign var='imgs' value=$product.tshirtecommerce_design_order_img|json_decode}
						{foreach from=$imgs key=ik item=r}
							html += "<div class='tshirtecommerce-img-thumb' href='{$link_img}{$r}' title='{$ik}' data-source='{$link_img}{$r}' product-name='{$product.product_name}' title=''>";
							html += "	<img alt='{$ik}' title='{$ik}' src='{$link_img}{$r}' width='88px' height='88px' class='tshirtecommerce-img-thumb-i' />";
							{if $tshirtecommerce_store_status == 1}
								{if $msg_store_empty == '' && count($store_temp)}
									...
									{/if}
								{else}
									html += '<br /><center><a target="_blank" class="tshirtecommerce-link-download" href="{$link_img}{$product.download_link}{$ik}">{l s="Download" mod="tshirtecommerce"}</a></center>';
								{/if}
							{else}
								html += '<br /><center><a target="_blank" class="tshirtecommerce-link-download" href="{$link_img}{$product.download_link}{$ik}">{l s="Download" mod="tshirtecommerce"}</a></center>';
							{/if}
			...
			//html += '</div>';
			if (link != '') {
				html += '<a href="'+link+'" target="_blank">{l s="View Design" mod="tshirtecommerce"}</a>';
			}
			...
			jQuery(this).find('.productName').parent().parent().append(html);
		});
	});
</script>